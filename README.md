...
Di menu **Kelayakan**, tombol **Unduh tabel pendataan (Excel)** menghasilkan lembar "Pendataan dan Verifikasi Syarat Awal Calon Penegak Garuda" (Nama, NTA/NIS, tanggal lantik Laksana, masa Laksana, tanggal lahir, usia, status kuota, saran aplikasi, dan kolom Verifikasi Pembina yang dikosongkan) untuk seluruh Calon dan Penegak yang SKU-nya selesai. Butuh migrasi `2026-09-snapshot-portofolio.sql`; **tanpa deploy Edge Function**.
Kode: `src/lib/snapshotLogic.js`, `src/lib/pendataanGarudaLogic.js`, `src/components/PanelSalinanBeku.jsx`; dijaga `uji/snapshot-portofolio.mjs`, `uji/pendataan-garuda.mjs`, `uji/migrasi-snapshot-portofolio.mjs`.
