...
Menu **Perlindungan** (Pembina dan Admin Gudep) mencatat kewajiban anggota dewasa gugus depan menurut Jukran Kwarnas 004/2021 tentang Perlindungan bagi Anggota Gerakan Pramuka: **Pembina** (pelatihan Safe From Harm, pakta integritas dan kode etik, pemeriksaan riwayat hidup dan rekam jejak) dan **Admin Gudep** (pelatihan). Tiap catatan berisi tanggal, tautan bukti opsional (berkasnya tidak disimpan di aplikasi), dan catatan singkat; mencatat ulang berarti koreksi. Halaman yang sama memuat **penerima laporan gugus depan** (nama, kontak, tautan prosedur, keterangan; Pasal 10 ayat 3) yang tampil pada kartu di **Akun saya** semua pengguna, agar Penegak tahu kepada siapa harus melapor.
- **Laporan kejadian TIDAK disimpan di SIGARDA**: laporan bersifat rahasia (Pasal 8 ayat 4 huruf g) dan ditangani Komite Perlindungan dan Dewan Kehormatan di luar aplikasi.
- Tabel `sfh_catatan` (unik per anggota dan jenis; RLS baca: pemilik dan Pembina/Admin; tulis hanya `sg_sfh_catat`, `sg_sfh_hapus`, `sg_sfh_gudep_simpan`; ikut cadangan). Penerima laporan pada pengaturan `perlindungan.gudep` (dibaca semua pengguna).
- **Periksa Data**: kategori "Catatan Safe From Harm belum lengkap" (`sfhBelum`) dengan tombol WhatsApp.
- Butuh migrasi `2026-09-perlindungan-anggota.sql`; tanpa deploy Edge Function. Kode: `perlindunganLogic.js` (cermin validasi, dibandingkan langsung dengan SQL), `useSfh.js`, `Perlindungan.jsx`, `KartuPerlindungan.jsx`. Dijaga `uji/perlindungan.mjs` dan `uji/migrasi-perlindungan-anggota.mjs`.
...
- [`2026-09-integrasi-pra-uji.sql`](supabase/migrasi/2026-09-integrasi-pra-uji.sql): **Fase E: integrasi pra-uji** dengan eskalasi, Periksa Data, dan pengingat. Hanya menulis ulang tiga fungsi (tanda tangan sama, tanpa tabel baru): `sigarda.eskalasi_mulai_sku` (pengingat "SKU tidak bergerak" tidak dihitung selama ada pra-uji yang menunggu penilai), `sigarda.pra_uji_pengingat` (pengingat "Pra-uji tanpa penilai" ke Pembina bertautan ke menu Pra-uji) dan `sg_pemeriksaan_data` (kunci baru `praUjiAktif`, `rombelTanpaBinaDamping`, `sanggaTanpaPinsa`, `praUjiMacet`; ditampilkan klien hanya bila pra-uji hidup). Jalankan **setelah** `2026-09-pra-uji.sql` (bila belum, berhenti dengan pesan yang menuntun). Tidak mengubah data dan tidak mengubah sakelar; Edge Function **tidak berubah**; aman diulang.
- [`2026-09-pelantikan-saka.sql`](supabase/migrasi/2026-09-pelantikan-saka.sql): **Tahap 2 (G1): pelantikan Bantara/Laksana dan keanggotaan Saka.** Tabel baru `public.pelantikan` (satu baris per Penegak per tingkat: tanggal, tempat, kegiatan Agenda opsional) dan `public.saka_anggota` (Saka, tanggal masuk, status aktif/selesai, tautan surat keterangan) dengan RLS baca (pemilik dan pengurus) dan pemicu `tolak_peserta_tak_aktif`; fungsi baru `sg_pelantikan_catat` (banyak Penegak sekaligus, semua atau tidak sama sekali), `sg_pelantikan_hapus`, `sg_saka_simpan`, `sg_saka_hapus` (Pembina dan Admin); dan penulisan ulang `sg_cadangan_admin()` (memuat tabel baru). Jalankan **setelah** `2026-09-integrasi-pra-uji.sql` (bila belum, berhenti dengan pesan yang menuntun). Tidak mengubah data; Edge Function **tidak berubah**; aman diulang.
- [`2026-09-tkk.sql`](supabase/migrasi/2026-09-tkk.sql): **Tahap 2 (G2): Tanda Kecakapan Khusus (TKK).** Tabel baru `public.tkk_katalog` (91 TKK: nama, bidang, golongan, sumber; isi sama dengan `src/data/tkkData.js`), `public.tkk_capaian` (capaian bertingkat Purwa, Madya, Utama per Penegak dengan tim penguji 2 orang, bukti melatih, tautan bukti) dan `public.tkk_krida`, dengan RLS baca (katalog semua pengguna aktif; capaian dan Krida pemilik dan pengurus) dan pemicu `tolak_peserta_tak_aktif`; ambang kesiapan Garuda bawaan pada pengaturan `tkk.ambang` (tidak menimpa bila sudah ada); fungsi baru `sg_tkk_catat`, `sg_tkk_hapus`, `sg_tkk_krida_simpan`, `sg_tkk_krida_hapus`, `sg_tkk_ambang_simpan` (Pembina dan Admin); dan penulisan ulang `sg_cadangan_admin()`. Jalankan **setelah** `2026-09-pelantikan-saka.sql` (bila belum, berhenti dengan pesan yang menuntun). Tidak mengubah data; Edge Function **tidak berubah**; aman diulang.
- [`2026-09-tkk-pengajuan.sql`](supabase/migrasi/2026-09-tkk-pengajuan.sql): **Tahap 2 (G2b): Penegak mengajukan TKK sendiri, Pembina meninjau.** Tabel baru `public.tkk_pengajuan` (status menunggu, disetujui, ditolak, dibatalkan; satu yang menunggu per Penegak, TKK, dan tingkat) dengan RLS baca (pemilik dan pengurus) dan pemicu `tolak_peserta_tak_aktif`; fungsi baru `sg_tkk_ajukan`, `sg_tkk_ajukan_batal` (Penegak) dan `sg_tkk_tinjau` (Pembina dan Admin: disetujui = menjadi capaian resmi, ditolak = catatan wajib); pemeriksa bersama `sigarda.tkk_periksa` yang kini juga dipakai `sg_tkk_catat` (ditulis ulang; perilaku dan pesan galat sama); jenis notifikasi baru `tkk` (kendala `notifikasi_jenis_check` diperbarui); dan penulisan ulang `sg_cadangan_admin()`. Jalankan **setelah** `2026-09-tkk.sql` (bila belum, berhenti dengan pesan yang menuntun). Tidak mengubah data; Edge Function **tidak berubah**; aman diulang.
- [`2026-09-tkk-penguji.sql`](supabase/migrasi/2026-09-tkk-penguji.sql): **Tahap 2 (G2c): Penguji 1 pengajuan TKK = Pembina yang ditugaskan; Pembina dapat mengganti penguji saat meninjau.** Kolom baru `tkk_pengajuan.penguji1_id` dan `penguji_awal`; fungsi baru `sigarda.tkk_pembina_penilai` dan `sg_tkk_penguji_pilihan`; **tanda tangan berubah**: `sg_tkk_ajukan` (Penguji 1 kini id Pembina yang dipilih dari daftar) dan `sg_tkk_tinjau` (boleh mengganti nama penguji dengan alasan wajib) — fungsi lama dibuang; notifikasi pengajuan baru hanya ke Pembina Penguji 1. Jalankan **setelah** `2026-09-tkk-pengajuan.sql` (bila belum, berhenti dengan pesan yang menuntun). Pengajuan lama tetap terbaca; tidak mengubah data; Edge Function **tidak berubah**; aman diulang.
- [`2026-09-spg.sql`](supabase/migrasi/2026-09-spg.sql): **Tahap 2 (G3): Syarat Pramuka Garuda (SPG, 13 butir).** Tabel baru `public.spg_penetapan` (satu baris per Penegak per butir: nilai 100/0, tanggal pengujian, catatan, tanda penimpaan hasil hitung aplikasi yang wajib beralasan; RLS baca pemilik dan pengurus, tulis hanya lewat fungsi; pemicu `tolak_peserta_tak_aktif`); fungsi baru `sg_spg_catat` dan `sg_spg_hapus` (Pembina dan Admin; Penegak aktif yang seluruh SKU Bantara dan Laksana-nya selesai); `sg_cadangan_admin()` ditulis ulang agar memuat tabel baru. Jalankan **setelah** `2026-09-tkk-penguji.sql` (bila belum, berhenti dengan pesan yang menuntun). Tidak mengubah data; Edge Function **tidak berubah**; aman diulang.
- [`2026-09-gerbang.sql`](supabase/migrasi/2026-09-gerbang.sql): **Tahap 2 (G4): gerbang calon Garuda.** Tabel baru `public.tanggal_lahir` (satu baris per Penegak, TERPISAH dari `profiles` agar tanggal lahir tidak terbaca Penegak lain; RLS baca pemilik dan pengurus, tulis hanya lewat fungsi; pemicu `tolak_peserta_tak_aktif`); aturan gerbang bawaan pada pengaturan `garuda.gerbang` (kelas minimal XI, lahir 1 Nov 2007 s.d. 1 Mei 2009, kuota 5%; tidak menimpa bila sudah ada); fungsi baru `sg_tanggal_lahir_atur` dan `sg_gerbang_simpan` (Pembina dan Admin); `sg_cadangan_admin()` ditulis ulang agar memuat tabel baru. Jalankan **setelah** `2026-09-spg.sql` (bila belum, berhenti dengan pesan yang menuntun). Tidak mengubah data; Edge Function **tidak berubah**; aman diulang.
- [`2026-09-tanggal-lahir-impor.sql`](supabase/migrasi/2026-09-tanggal-lahir-impor.sql): **Tahap 2 (G4b): impor tanggal lahir Penegak dari template Excel.** Hanya menambah fungsi `sg_tanggal_lahir_impor` (Pembina dan Admin; banyak Penegak sekaligus dari kolom "Tanggal Lahir" template, semua atau tidak sama sekali). Tidak mengubah tabel maupun data. Jalankan **setelah** `2026-09-gerbang.sql` (bila belum, berhenti dengan pesan yang menuntun). Edge Function **tidak berubah**; aman diulang.
- [`2026-09-tim-kalender.sql`](supabase/migrasi/2026-09-tim-kalender.sql): **Tahap 2 (G4b dan G4c): tim penilai Calon Garuda dan kalender tahap Garuda dari Kwarcab.** Tabel baru `public.tim_penilai` (satu tim per tahun ajaran per putra/putri; nomor dan tanggal SK berpasangan), `public.tim_penilai_anggota` (1-15 anggota; paling banyak satu ketua) dan `public.garuda_tahap` (tanggal mulai dan akhir tiap tahap seleksi); RLS baca pengurus, tulis hanya lewat fungsi; fungsi baru `sg_tim_penilai_simpan/_hapus` dan `sg_garuda_tahap_simpan/_hapus` (Pembina dan Admin); `sg_cadangan_admin()` ditulis ulang (daftar tabelnya dibagi dua objek karena PostgreSQL membatasi 100 argumen per fungsi). Jalankan **setelah** `2026-09-tanggal-lahir-impor.sql` (bila belum, berhenti dengan pesan yang menuntun). Tidak mengubah data; Edge Function **tidak berubah**; aman diulang.
- [`2026-09-pengingat-kalender.sql`](supabase/migrasi/2026-09-pengingat-kalender.sql): **Tahap 2 (G4d): pengingat otomatis untuk kalender tahap Garuda.** Fungsi baru `sigarda.garuda_tahap_label` dan `sigarda.garuda_kalender_pengingat` (notifikasi H-7, H-3, H-1, hari-H mulai, dan "berakhir besok" untuk tahap berentang, ke semua pengurus aktif dan Penegak berjabatan Dewan, sekali per tahap per hari) dan `sigarda.notif_pengingat()` ditulis ulang (tanda tangan sama) agar memanggilnya. Tidak mengubah tabel maupun data. Jalankan **setelah** `2026-09-tim-kalender.sql` (bila belum, berhenti dengan pesan yang menuntun). Edge Function **tidak berubah**; aman diulang.
- [`2026-09-cakupan-pra-uji.sql`](supabase/migrasi/2026-09-cakupan-pra-uji.sql): **Tahap 2 (G4e): cakupan pra-uji (tindak lanjut simulasi pra-uji).** Hanya menambah fungsi `sg_pra_uji_cakupan` (Pembina dan Admin; membaca): berapa pengajuan baru yang melewati pra-uji dibanding yang langsung ke antrian Pembina karena tidak ada penilai, per rombel, beserta jumlah Bina Damping rombel itu. Tidak mengubah tabel maupun data. Jalankan **setelah** `2026-09-pengingat-kalender.sql` (bila belum, berhenti dengan pesan yang menuntun). Edge Function **tidak berubah**; aman diulang.
- [`2026-09-isian-penegak.sql`](supabase/migrasi/2026-09-isian-penegak.sql): **Tahap 3 (H1): isian data diri Penegak (diisi sendiri) dan templat surat keterangan guru.** Melonggarkan batasan `profil_peserta` (Penegak baru hanya butuh NIS dan rombel; sangga dan agama boleh kosong); menambah tabel `penegak_isian` dan `dokumen_templat` (RLS baca, hak tulis dicabut, pemicu tak aktif), fungsi `sigarda.isian_periksa`, `sigarda.isian_periksa_profil`, `sg_isian_saya_simpan` (Penegak aktif untuk dirinya), `sg_dokumen_templat_simpan/_hapus` (Pembina dan Admin); menulis ulang (tanda tangan sama) `sigarda.tolak_peserta_tak_aktif` (penjaga: tanpa agama, progres SKU ditolak), `sigarda.sangga_peringatan`, `sg_sangga_atur` (mengenal Penegak tanpa sangga), `sg_anggota_ubah` (sangga dan agama opsional), dan `sg_cadangan_admin`. Jalankan **setelah** `2026-09-cakupan-pra-uji.sql` (bila belum, berhenti dengan pesan yang menuntun). **Edge Function `sigarda` BERUBAH** (sangga dan agama Penegak tidak lagi wajib saat membuat akun): **deploy ulang Edge Function `sigarda`** (timpa). Jalankan migrasi sebelum deploy dan sebelum `git push`. Tidak menghapus data; aman diulang.
- [`2026-09-snapshot-portofolio.sql`](supabase/migrasi/2026-09-snapshot-portofolio.sql): **Tahap 3 (H3): salinan beku Portofolio format Kwarcab.** Tabel baru `public.portofolio_snapshot` (RLS baca Pembina dan Admin, hak tulis dicabut, indeks `portofolio_snapshot_peserta_idx`), fungsi `sg_portofolio_snapshot_simpan` (Pembina dan Admin; paling banyak 600 kB dan 20 salinan per Penegak) dan `sg_portofolio_snapshot_hapus`, dan `sg_cadangan_admin` ditulis ulang (tanda tangan sama) agar memuat tabel baru. Jalankan **setelah** `2026-09-isian-penegak.sql` (bila belum, berhenti dengan pesan yang menuntun). Edge Function **tidak berubah**. Tidak menghapus data; aman diulang.
- [`2026-09-periksa-data-diri.sql`](supabase/migrasi/2026-09-periksa-data-diri.sql): **Tahap 3 (H1, lanjutan): Pemeriksaan Data memuat kategori "Penegak belum melengkapi data diri".** Hanya menulis ulang `sg_pemeriksaan_data()` (tanda tangan sama): kunci baru `dataDiriBelum` berisi Penegak aktif yang belum mengisi isian pokok (WhatsApp, jenis kelamin, agama, tanggal lahir, tempat lahir, alamat, nama ayah/ibu/wali); yang dikembalikan hanya nama dan KODE isian yang kurang, tidak pernah nilainya. Jalankan **setelah** `2026-09-snapshot-portofolio.sql` (bila belum, berhenti dengan pesan yang menuntun). Edge Function **tidak berubah**. Tidak mengubah tabel maupun data; aman diulang.
- [`2026-09-perlindungan-anggota.sql`](supabase/migrasi/2026-09-perlindungan-anggota.sql): **Tahap 4: Perlindungan anggota / Safe From Harm.** Tabel baru `sfh_catatan` (catatan pelatihan, pakta integritas, dan rekam jejak anggota dewasa; RLS baca pemilik dan Pembina/Admin, hak tabel dicabut), fungsi `sg_sfh_catat`, `sg_sfh_hapus`, `sg_sfh_gudep_simpan` (penerima laporan gugus depan pada pengaturan `perlindungan.gudep`), serta menulis ulang `sg_pemeriksaan_data()` (kategori baru `sfhBelum`) dan `sg_cadangan_admin()` (memuat `sfh_catatan`). Jalankan **setelah** `2026-09-periksa-data-diri.sql` (bila belum, berhenti dengan pesan yang menuntun). Edge Function **tidak berubah**. Tidak mengubah data yang ada; aman diulang.
- [`2026-09-pemeriksaan-jumlah.sql`](supabase/migrasi/2026-09-pemeriksaan-jumlah.sql): **Periksa Data menampilkan jumlah sebenarnya (temuan simulasi beban).** Hanya menulis ulang `sg_pemeriksaan_data()` (tanda tangan sama): daftar tetap dipotong di 300 baris, tetapi hasil kini memuat kunci `jumlahSebenarnya` (`kelasLama`, `tanpaNta`, `tanpaJk`, `dataDiriBelum`, `belumPernahMasuk`), dihitung hanya bila daftarnya penuh. Layar menampilkan "684 ditemukan. Daftar menampilkan 300 yang pertama." Jalankan **setelah** `2026-09-perlindungan-anggota.sql` (bila belum, berhenti dengan pesan yang menuntun). Edge Function **tidak berubah**. Tidak mengubah tabel maupun data; aman diulang.
- [`2026-09-pinsa-tugas.sql`](supabase/migrasi/2026-09-pinsa-tugas.sql): **Pinsa tertugas lintas rombel (persiapan uji coba pra-uji 2 Oktober 2026).** Tabel baru `pinsa_tugas` (Penegak Calon Laksana yang ditugaskan menjadi Pinsa sebuah sangga di rombel lain; satu orang satu sangga per tahun ajaran, paling banyak 2 per sangga; hak baca langsung dicabut), fungsi baru `sg_pinsa_calon`, `sg_pinsa_tugaskan`, `sg_pinsa_cabut` dan `sigarda.pinsa_tugas_bersihkan`, serta menulis ulang (tanda tangan sama) `pra_uji_penilai_ok`, `pra_uji_penilai_daftar`, `sangga_peringatan`, `sg_sangga_rombel`, `sg_pendampingan_saya`, `sg_pemeriksaan_data`, dan `sg_cadangan_admin`. Jalankan **setelah** `2026-09-pemeriksaan-jumlah.sql` (bila belum, berhenti dengan pesan yang menuntun). Edge Function **tidak berubah**. Tidak mengubah data yang ada; aman diulang.
...
│   ├── sumber/*.sql                    sumber skema tanpa katalog, 38 berkas bernomor per modul (EDIT DI SINI, lalu npm run skema; peta di sumber/README.md)
...
[`supabase/demo/hapus_semua_akun_kecuali_admin.sql`](supabase/demo/hapus_semua_akun_kecuali_admin.sql) (bawaannya menolak berjalan sampai `v_konfirmasi` diubah menjadi `'YA'`; satu pernyataan, semua atau tidak sama sekali; struktur tidak disentuh; dijaga pengujian `hapus-akun`). Buat cadangan lebih dulu. Bawaannya mengosongkan **total**: selain akun dan data milik akun (progres SKU, pra-uji, Bina Damping, pelantikan, Saka, TKK, SPG, tanggal lahir dan data diri, salinan beku portofolio, catatan Safe From Harm), ikut terhapus agenda, usulan kegiatan, sesi latihan/ujian, dokumen terbit, riwayat, tim penilai dan kalender Garuda, pengukuhan Dewan, **serta pengaturan (Data Gudep, format nomor, iuran, ambang TKK), templat surat keterangan guru, materi SKU, instrumen penilaian, dan guru agama**. Yang tetap: kedua Admin Gudep beserta PIN-nya, katalog butir SKU/dokumen portofolio/TKK, konfigurasi Web Push, dan catatan Safe From Harm milik Admin. Untuk mempertahankan sebagian, ubah `v_hapus_kegiatan`, `v_hapus_pengaturan`, `v_hapus_materi`, `v_hapus_instrumen`, atau `v_hapus_guru_agama` menjadi `false` di bagian PENGATURAN skrip. **Pengaman tabel baru:** skrip menolak berjalan (tanpa mengubah apa pun) bila skema `public` punya tabel yang belum dikenalnya, jadi tabel yang ditambah migrasi kelak tidak tertinggal diam-diam: daftarkan tabel itu di skrip dan pratinjau lebih dulu. Tabel dari migrasi yang belum dijalankan dilewati.
