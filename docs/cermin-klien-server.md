...
| Boleh menguji, Penegak berjabatan | `rombelLogic.bisaMenguji(u, praUji)`, `penegakDewan` | `sigarda.bisa_menguji` (bergantung sakelar `pra_uji.aktif`: hidup = hanya Pembina; dan klausa `jabatan_dewan is not null` pada `pengurus`/`dewan`) | hak | `paritas-hak` (kedua keadaan sakelar), `dewan-penegak` |
| Penguji sah / peran penguji / ditugaskan | `rombelLogic.pengujiSah`, `pengujiPeranOk`, `ditugaskanUntuk` (parameter `praUji` = sakelar `pra_uji.aktif`, disalurkan juga ke `daftarPengujiUrut`, `bolehMenilaiPoin`, `antrianPengujian`) | `sigarda.penguji_sah/penguji_peran_ok/ditugaskan` | hak | `penegakan` (perbandingan menyeluruh klien = server, juga dengan sakelar pra-uji hidup) |
| Syarat dan isian pelantikan/Saka (Penegak aktif, SKU tingkat selesai, tanggal, tempat, catatan, nama Saka, status, tautan surat) | `pelantikanLogic.calonPelantikan/periksaPelantikan/periksaSaka` (memakai `skuLogic.tingkatSelesai`) | `sg_pelantikan_catat`, `sg_saka_simpan` (dan `sigarda.tingkat_selesai`) | tampilan (server menegakkan) | `pelantikan-klien` (kisi masukan dibandingkan langsung dengan SQL: menerima/menolak sama) |
| Isian TKK (jenis, agama, tingkat, tanggal, tim penguji, bukti melatih, tautan, catatan), Krida, dan ambang kesiapan Garuda | `tkkLogic.periksaCapaian/periksaKrida/periksaAmbang` (katalog `src/data/tkkData.js` = tabel `tkk_katalog`) | `sg_tkk_catat`, `sg_tkk_krida_simpan`, `sg_tkk_ambang_simpan` | tampilan (server menegakkan) | `tkk-klien` (kisi masukan dibandingkan langsung dengan SQL) dan `tkk` (katalog klien = tabel) |
| Isian peninjauan pengajuan TKK (keputusan dan catatan; menolak wajib bercatatan) | `tkkLogic.periksaTinjau` | `sg_tkk_tinjau` | tampilan (server menegakkan) | `tkk-pengajuan-klien` (kisi masukan dibandingkan langsung dengan SQL) |
| Isian penetapan SPG (butir 1-13, nilai 100/0, tanggal pengujian, catatan, alasan bila berbeda dari hasil aplikasi) | `spgLogic.periksaSpg` (hasil hitung otomatis dan saran dokumen `hitungSpg` HANYA di klien, tanpa cermin) | `sg_spg_catat` | tampilan (server menegakkan) | `spg-klien` (kisi masukan dibandingkan langsung dengan SQL) |
| Isian tanggal lahir Penegak dan aturan gerbang calon Garuda (kelas minimal, rentang lahir, kuota) | `gerbangLogic.periksaTanggalLahir/periksaGerbang` (syarat gerbang `hitungGerbang/kuotaCalon` HANYA di klien, tanpa cermin; gerbang hanya peringatan) | `sg_tanggal_lahir_atur`, `sg_gerbang_simpan` | tampilan (server menegakkan) | `gerbang-klien` (kisi masukan dibandingkan langsung dengan SQL, termasuk bentuk rusak) |
| Isian tim penilai (tahun ajaran, putra/putri, nomor dan tanggal SK berpasangan, tautan, anggota 1-15 dengan unsur dan satu ketua) dan tahap kalender Garuda (tahun ajaran, tahap dari daftar, mulai, akhir, catatan) | `timLogic.periksaTim`, `kalenderGarudaLogic.periksaTahap` (daftar `UNSUR_TIM` dan `TAHAP_GARUDA` = batasan check tabel; peringatan komposisi dan status kalender HANYA di klien) | `sg_tim_penilai_simpan`, `sg_garuda_tahap_simpan` | tampilan (server menegakkan) | `tim-kalender-klien` (kisi masukan dibandingkan langsung dengan SQL) |
| Tanggal lahir dari kolom template import (bentuk isian, rentang) | `importAnggota.normalisasiTanggalLahir` lalu `gerbangLogic.periksaTanggalLahir` (bentuk ISO dikirim ke server) | `sg_tanggal_lahir_impor` | tampilan (server menegakkan) | `import-lahir` (klien) dan `gerbang-klien` (rentang, dibandingkan dengan SQL) |
| Urutan tahap pra-uji per tingkat (Bantara: Pinsa, Bina Damping, Pembina; Laksana: Bina Damping, Pembina) | `praUjiLogic.jalurTahap` (tampilan jalur per butir, `jalurPraUji`) | `sigarda.pra_uji_tahap_berikut` | tampilan (server yang memutuskan tahap) | `pra-uji-klien` (dibandingkan langsung dengan SQL) |
