...
console.log('\n--- Sebelum L0: notifikasi uji dan pemeriksaan data belum ada; migrasi menyembuhkan ---');
{
  ...
  ok(m.length === 109 && m.some((x) => /sg_notifikasi_tes/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /notifikasi_jenis_check/.test(x.objek) && x.status === 'BEDA')
      && m.some((x) => /sg_pemeriksaan_data/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /sg_cadangan_admin/.test(x.objek) && x.status === 'KURANG')
    ...
    `${m.length} temuan (tabel agenda, kegiatan_usulan, dan garuda_berkas_token baru membawa banyak kolom/batasan/indeks/kebijakan sekaligus): notifikasi uji, pemeriksaan data, cadangan (L4), eskalasi (L5), agenda (L6), usulan kegiatan (L6b), dan berkas Calon Garuda (L7) belum ada: ` + JSON.stringify(m.map((x) => x.objek)));
  const migrasiL0L6b = ['2026-09-tes-notifikasi', '2026-09-pemeriksaan-data', '2026-09-cadangan', '2026-09-eskalasi', '2026-09-agenda', '2026-09-usulan-kegiatan', '2026-09-berkas-garuda'];
  for (const nama of migrasiL0L6b) await S.exec(bersih(readFileSync(`${P}/supabase/migrasi/${nama}.sql`, 'utf8')));
  ok(masalah(await jalankan(S)).length === 0, 'sesudah migrasi tes-notifikasi, pemeriksaan-data, cadangan, eskalasi, agenda, usulan-kegiatan, dan berkas-garuda: tidak ada masalah');
  for (const nama of migrasiL0L6b) await S.exec(bersih(readFileSync(`${P}/supabase/migrasi/${nama}.sql`, 'utf8')));
