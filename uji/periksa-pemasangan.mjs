...
console.log('\n--- Sebelum L0: notifikasi uji dan pemeriksaan data belum ada; migrasi menyembuhkan ---');
{
  ...
  ok(m.length === 630 && m.some((x) => x.objek === "portofolio_snapshot" && x.status === "KURANG") && m.some((x) => x.objek === "dokumen_templat" && x.status === "KURANG") && m.some((x) => /sg_isian_saya_simpan/.test(x.objek) && x.status === "KURANG") && m.some((x) => /sg_notifikasi_tes/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /notifikasi_jenis_check/.test(x.objek) && x.status === 'BEDA')
      ...
      && m.some((x) => x.objek === 'pelantikan' && x.status === 'KURANG') && m.some((x) => x.objek === 'saka_anggota' && x.status === 'KURANG') && m.some((x) => /sg_pelantikan_catat/.test(x.objek) && x.status === 'KURANG')
      && m.some((x) => x.objek === 'tkk_katalog' && x.status === 'KURANG') && m.some((x) => x.objek === 'tkk_capaian' && x.status === 'KURANG') && m.some((x) => /sg_tkk_catat/.test(x.objek) && x.status === 'KURANG')
      && m.some((x) => x.objek === 'tkk_pengajuan' && x.status === 'KURANG') && m.some((x) => /sg_tkk_tinjau/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /sg_tkk_penguji_pilihan/.test(x.objek) && x.status === 'KURANG') && m.some((x) => x.objek === 'spg_penetapan' && x.status === 'KURANG') && m.some((x) => /sg_spg_catat/.test(x.objek) && x.status === 'KURANG') && m.some((x) => x.objek === 'tanggal_lahir' && x.status === 'KURANG') && m.some((x) => /sg_gerbang_simpan/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /sg_tanggal_lahir_impor/.test(x.objek) && x.status === 'KURANG') && m.some((x) => x.objek === 'tim_penilai' && x.status === 'KURANG') && m.some((x) => x.objek === 'garuda_tahap' && x.status === 'KURANG') && m.some((x) => /sg_tim_penilai_simpan/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /garuda_kalender_pengingat/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /sg_pra_uji_cakupan/.test(x.objek) && x.status === 'KURANG'),
    `${m.length} temuan (tabel agenda, kegiatan_usulan, dan garuda_berkas_token baru membawa banyak kolom/batasan/indeks/kebijakan sekaligus): notifikasi uji, pemeriksaan data, cadangan (L4), eskalasi (L5), agenda (L6), usulan kegiatan (L6b), berkas Calon Garuda (L7), pengukuhan Dewan (Fase A), Pinsa dan Bina Damping (Fase B), pra-uji (Fase C), pelantikan dan Saka (Tahap 2), TKK (Tahap 2 G2), pengajuan TKK (G2b), penguji TKK (G2c), SPG (G3), gerbang calon (G4), impor tanggal lahir dan tim penilai serta kalender (G4b dan G4c), sg_push_ringkasan versi lama (periksa-dewan), , 10 indeks kunci asing, dan keep-alive dari dalam database, serta Pinsa dan Bina Damping (fase B) belum ada/berbeda: ` + JSON.stringify(m.map((x) => x.objek)));
  const migrasiL0L6b = ['2026-09-tes-notifikasi', '2026-09-pemeriksaan-data', '2026-09-cadangan', '2026-09-eskalasi', '2026-09-agenda', '2026-09-usulan-kegiatan', '2026-09-berkas-garuda', '2026-09-periksa-dewan', '2026-09-indeks-fk', '2026-09-keepalive', '2026-09-pinsa-bina-damping', '2026-09-pengukuhan-dewan', '2026-09-pra-uji', '2026-09-integrasi-pra-uji', '2026-09-pelantikan-saka', '2026-09-tkk', '2026-09-tkk-pengajuan', '2026-09-tkk-penguji', '2026-09-spg', '2026-09-gerbang', '2026-09-tanggal-lahir-impor', '2026-09-tim-kalender', '2026-09-pengingat-kalender', '2026-09-cakupan-pra-uji', '2026-09-isian-penegak', '2026-09-snapshot-portofolio', '2026-09-periksa-data-diri', '2026-09-perlindungan-anggota', '2026-09-pemeriksaan-jumlah', '2026-09-pinsa-tugas'];
  for (const nama of migrasiL0L6b) await S.exec(bersih(readFileSync(`${P}/supabase/migrasi/${nama}.sql`, 'utf8')));
  ok(masalah(await jalankan(S)).length === 0, 'sesudah migrasi tes-notifikasi, pemeriksaan-data, cadangan, eskalasi, agenda, usulan-kegiatan, berkas-garuda, periksa-dewan, indeks-fk, keepalive, pinsa-bina-damping, pengukuhan-dewan, pra-uji, integrasi-pra-uji, pelantikan-saka, tkk, tkk-pengajuan, tkk-penguji, spg, gerbang, tanggal-lahir-impor, tim-kalender, pengingat-kalender, cakupan-pra-uji, isian-penegak, snapshot-portofolio, periksa-data-diri, perlindungan-anggota, pemeriksaan-jumlah, dan pinsa-tugas: tidak ada masalah');
  for (const nama of migrasiL0L6b) await S.exec(bersih(readFileSync(`${P}/supabase/migrasi/${nama}.sql`, 'utf8')));
