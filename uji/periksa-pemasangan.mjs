...
console.log('\n--- Sebelum L0: notifikasi uji dan pemeriksaan data belum ada; migrasi menyembuhkan ---');
{
  ...
  ok(m.length === 424 && m.some((x) => /sg_notifikasi_tes/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /notifikasi_jenis_check/.test(x.objek) && x.status === 'BEDA')
      ...
      && m.some((x) => x.objek === 'tkk_pengajuan' && x.status === 'KURANG') && m.some((x) => /sg_tkk_tinjau/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /sg_tkk_penguji_pilihan/.test(x.objek) && x.status === 'KURANG'),
    `${m.length} temuan (tabel agenda, kegiatan_usulan, dan garuda_berkas_token baru membawa banyak kolom/batasan/indeks/kebijakan sekaligus): notifikasi uji, pemeriksaan data, cadangan (L4), eskalasi (L5), agenda (L6), usulan kegiatan (L6b), berkas Calon Garuda (L7), pengukuhan Dewan (Fase A), Pinsa dan Bina Damping (Fase B), pra-uji (Fase C), pelantikan dan Saka (Tahap 2), TKK (Tahap 2 G2), pengajuan TKK (G2b), penguji TKK (G2c), sg_push_ringkasan versi lama (periksa-dewan), , 10 indeks kunci asing, dan keep-alive dari dalam database, serta Pinsa dan Bina Damping (fase B) belum ada/berbeda: ` + JSON.stringify(m.map((x) => x.objek)));
