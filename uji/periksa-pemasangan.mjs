...
console.log('\n--- Sebelum L0: notifikasi uji dan pemeriksaan data belum ada; migrasi menyembuhkan ---');
{
  ...
  ok(m.length === 609 && m.some((x) => x.objek === "portofolio_snapshot" && x.status === "KURANG") && m.some((x) => x.objek === "dokumen_templat" && x.status === "KURANG") && m.some((x) => /sg_isian_saya_simpan/.test(x.objek) && x.status === "KURANG") && m.some((x) => /sg_notifikasi_tes/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /notifikasi_jenis_check/.test(x.objek) && x.status === 'BEDA')
