...
console.log('\n--- Sebelum L0: notifikasi uji dan pemeriksaan data belum ada; migrasi menyembuhkan ---');
{
  ...
  ok(m.length === 297 && m.some((x) => /sg_notifikasi_tes/.test(x.objek) && x.status === 'KURANG') && m.some((x) => /notifikasi_jenis_check/.test(x.objek) && x.status === 'BEDA')
    ...
    `${m.length} temuan (tabel agenda, kegiatan_usulan, dan garuda_berkas_token baru membawa banyak kolom/batasan/indeks/kebijakan sekaligus): notifikasi uji, pemeriksaan data, cadangan (L4), eskalasi (L5), agenda (L6), usulan kegiatan (L6b), berkas Calon Garuda (L7), pengukuhan Dewan (Fase A), Pinsa dan Bina Damping (Fase B), pra-uji (Fase C), pelantikan dan Saka (Tahap 2), sg_push_ringkasan versi lama (periksa-dewan), , 10 indeks kunci asing, dan keep-alive dari dalam database, serta Pinsa dan Bina Damping (fase B) belum ada/berbeda: ` + JSON.stringify(m.map((x) => x.objek)));
  const migrasiL0L6b = ['2026-09-tes-notifikasi', '2026-09-pemeriksaan-data', '2026-09-cadangan', '2026-09-eskalasi', '2026-09-agenda', '2026-09-usulan-kegiatan', '2026-09-berkas-garuda', '2026-09-periksa-dewan', '2026-09-indeks-fk', '2026-09-keepalive', '2026-09-pinsa-bina-damping', '2026-09-pengukuhan-dewan', '2026-09-pra-uji', '2026-09-integrasi-pra-uji', '2026-09-pelantikan-saka'];
  for (const nama of migrasiL0L6b) await S.exec(bersih(readFileSync(`${P}/supabase/migrasi/${nama}.sql`, 'utf8')));
  ok(masalah(await jalankan(S)).length === 0, 'sesudah migrasi tes-notifikasi, pemeriksaan-data, cadangan, eskalasi, agenda, usulan-kegiatan, berkas-garuda, periksa-dewan, indeks-fk, keepalive, pinsa-bina-damping, pengukuhan-dewan, pra-uji, integrasi-pra-uji, dan pelantikan-saka: tidak ada masalah');
  for (const nama of migrasiL0L6b) await S.exec(bersih(readFileSync(`${P}/supabase/migrasi/${nama}.sql`, 'utf8')));
