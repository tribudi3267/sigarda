...
- **Identitas gudep dan pejabat (nama gudep, ambalan, nomor gudep, Pembina/Ka Gudep, Kamabigus, Pradana, Pradani, NTA) DIATUR ADMIN di menu "Data Gudep"**, bukan di kode: baca lewat `useGudep()` (komponen) atau `ambilGudep()` (non-React) dari `src/lib/gudepStore.js`; jangan mengimpor `GUDEP_BAWAAN` (`src/config.js`) di halaman (hanya nilai awal). Tersimpan di pengaturan `gudep.data` (`sg_gudep_simpan`, Admin); tanpa login hanya `sg_gudep_publik` (nama gudep, ambalan, sekolah, kota). Aturan isian: `src/lib/gudepLogic.js` (dijaga `uji/gudep.mjs`). Dokumen baru wajib memakai data ini untuk kop, kota, dan penanda tangan. Ketua sidang = Pradana (`sigarda.ketua_sidang`, disalin ke catatan sidang; `sidang.nama_ketua/sebutan_ketua` hanya cadangan).
