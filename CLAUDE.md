...
- Pengujian migrasi memakai skema lama dari git: `baru('git:<commit>')` = `supabase/skema.sql` pada commit itu (pakai commit TEPAT sebelum perubahan). Contoh: `uji/migrasi-iuran.mjs`. Pengujian `instrumen` dilewati kecuali `ISI_INSTRUMEN=<jalur SQL isi instrumen>`.
